...
class XsBitArray(BitArray):
    ...
    # least-significant bit (0 in this case) in the highest index position (5
    # in this case).
    #
    ...
    def pop_field(self, length):
        """
        Remove the first set of transmitted or received bits from a bitstring
        and return it.
        """
    ...
    def to_usb(self):
        """
        Convert a bitstring into a byte array with the bits in each byte ordered
        correctly for transmission over USB to an XESS board.
        """
        # The bit strings are stored with the first transmitted bit at the
        # highest index like so:
        #   XsBitArray order:
        #     | b15 b14 b13 b12 b11 b10 b9 b8 | b7 b6 b5 b4 b3 b2 b1 b0 |
        # But the XESS board expects to get a USB packet with bytes where the
        # least-significant bit of the first byte is the first bit to transmit:
        #   USB buffer bit order:
        #     | b7 b6 b5 b4 b3 b2 b1 b0 | b15 b14 b13 b12 b11 b10 b9 b8 |
        # So this function pads the bit string so it consists of complete bytes,
        # converts the bitstring into bytes, and finally reverses the order of
        # the bytes.
        # Pad the bitstring to make complete bytes.
        bits = self + XsBitArray((8 - self.len % 8) % 8)
        # Convert bit string to bytes and reverse their order.
        return bits.tobytes()[::-1]

    @staticmethod
    def from_usb(usb_bytes, length=0):
        """
        Create a bitstring from a byte array received over USB so that the bit
        ordering of the bytes is correct.
        """
        # The bytes sent by XESS boards contain the first received bit in the
        # least-significant bit of the first byte as follows:
        #   USB buffer bit order:
        #     | b7 b6 b5 b4 b3 b2 b1 b0 | b15 b14 b13 b12 b11 b10 b9 b8 |
        # This has to be converted into a bit string with the first received bit
        # at the highest index like so:
        #   XsBitArray order:
        #     | b15 b14 b13 b12 b11 b10 b9 b8 | b7 b6 b5 b4 b3 b2 b1 b0 |
        # So this function reverses the byte order, creates a bit string, and
        # then cuts it to length.
        # Create a bit string from the reversed USB bytes.
        bits = XsBitArray(bytes=usb_bytes[::-1])
        return bits[-length:]
