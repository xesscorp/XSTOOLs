...
class XsJtag:
    ...
    def shift_tdo(self, num_bits, do_exit_shift=False):
        ...
        if do_exit_shift == True:
            ...
            tdo_bits = XsBitArray.from_usb_buffer(buffer, num_bits)
    ...
    def flush(self):
        ...
        if self._tdi_bits.len == 0:
            ...
            if self._tms_bits.len == 0:
                ...
                elif self._tms_bits.len == 1:
                    # There's multiple TDI bits but only one TMS bit.
                    # Remove the last TMS and TDI bits from the buffers.
                    last_tms_bit = self._tms_bits[-1]
                    self._tms_bits = self._tms_bits[:-1]
                    last_tdi_bit = self._tdi_bits[-1]
                    self._tdi_bits = self._tdi_bits[:-1]
    ...
    def load_ir_then_dr(
        ...
        if data != None:
            ...
            self.go_thru_tap_states('Update-DR')
        assert self._tap_state == 'Run-Test/Idle' \
            or self._tap_state == 'Update-IR' \
            or self._tap_state == 'Update-DR'
        self.go_thru_tap_states('Run-Test/Idle')
        self.flush()
        return bits
