...
class PicMicro:
    ...
    def erase_flash(self, bottom=None, top=None):
        ...
        top = int((top + self.ERASE_BLOCK_SIZE - 1)/self.ERASE_BLOCK_SIZE) * self.ERASE_BLOCK_SIZE
        assert bottom <= top, "Bottom address is greater than the top address."
        for address in range(bottom, top, self.ERASE_BLOCK_SIZE):
            self.xsusb.erase_flash(address)

    def write_flash(self, hexfile, bottom=None, top=None):
        ...
        if type(hexfile) is str:
            try:
                ...
                raise XsMajorError("Unable to open hex file %s for writing to uC flash." % hexfile)
        bottom = bottom & ((1<<32) - self.WRITE_BLOCK_SIZE)
        top = int((top + self.WRITE_BLOCK_SIZE - 1)/self.WRITE_BLOCK_SIZE) * self.WRITE_BLOCK_SIZE
        assert bottom <= top, "Bottom address is greater than the top address."
        for address in range(bottom, top, self.WRITE_BLOCK_SIZE):
            ...
            self.xsusb.write_flash(address, bytearray(hexfile.tobinarray(start=address, size=self.WRITE_BLOCK_SIZE)))
    ...
    def read_flash(self, bottom=None, top=None):
        ...
        for address in range(bottom, top, self.READ_BLOCK_SIZE):
            data.extend(self.xsusb.read_flash(address, self.READ_BLOCK_SIZE))
        hex_data = IntelHex()
        hex_data[bottom:top] = [byte for byte in data]
        return hex_data
        
    def verify_flash(self, hexfile, bottom=None, top=None):
        ...
        if type(hexfile) is str:
            try:
                ...
                raise XsMajorError("Unable to open hex file %s for verifying uC flash." % hexfile)
        bottom = bottom & ((1<<32) - self.WRITE_BLOCK_SIZE)
        top = int((top + self.WRITE_BLOCK_SIZE - 1)/self.WRITE_BLOCK_SIZE) * self.WRITE_BLOCK_SIZE
        assert bottom <= top, "Bottom address is greater than the top address."
        flash = self.read_flash(bottom, top)
        errors = [(a, flash[a], hexfile[a]) for a in sorted(hexfile.todict().keys()) if flash[a] != hexfile[a] and bottom<=a<top]
        if len(errors) > 0:
            raise XsMajorError("uC flash != hex file at %d locations starting at address 0x%04x (0x%02x != 0x%02x)" %
                (len(errors),errors[0][0],errors[0][1],errors[0][2]))
        
    def program_flash(self, hexfile, bottom=None, top=None):
